...
const initTables = async (connection) => {
  const tableDefinitions = [
    ...
    `CREATE TABLE IF NOT EXISTS educations (
      ...
      location VARCHAR(255),
      order_num INT DEFAULT 0,
      catgry VARCHAR(100) DEFAULT 'Education',
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;`,
  ...
  // Ensure category and catgry columns exist on educations table for existing tables
  try {
    const [eduCols] = await connection.query("SHOW COLUMNS FROM educations LIKE 'category'");
    if (eduCols.length === 0) {
      await connection.query("ALTER TABLE educations ADD COLUMN category VARCHAR(100) DEFAULT 'Education'");
      console.log("[MySQL] Added category column to educations table");
    }
  } catch (colErr) {
    console.warn("[MySQL] Column category on educations migration notice:", colErr.message);
  }

  try {
    const [eduCols2] = await connection.query("SHOW COLUMNS FROM educations LIKE 'catgry'");
    if (eduCols2.length === 0) {
      await connection.query("ALTER TABLE educations ADD COLUMN catgry VARCHAR(100) DEFAULT 'Education'");
      console.log("[MySQL] Added catgry column to educations table");
    }
  } catch (colErr) {
    console.warn("[MySQL] Column catgry on educations migration notice:", colErr.message);
  }
};
...
export const db = {
  ...
  async getEducations() {
    if (isUsingRealMysql && pool) {
      ...
      return rows.map((r) => {
        const cat = r.category || r.catgry || (r.degree && r.degree.toLowerCase().includes("cert") ? "Certificate" : "Education");
        return {
          _id: r.id,
          id: r.id,
          ...
          field: r.field,
          startDate: r.start_date,
          endDate: r.end_date !== undefined && r.end_date !== null ? r.end_date : "",
          description: r.description,
          location: r.location,
          order: r.order_num,
          category: cat,
    ...
    return [...memoryStore.educations].map((e) => {
      const cat = e.category || e.catgry || (e.degree && e.degree.toLowerCase().includes("cert") ? "Certificate" : "Education");
      return {
        ...e,
        endDate: e.endDate !== undefined && e.endDate !== null ? e.endDate : "",
        category: cat,
        catgry: cat,
      };
    }).sort((a, b) => (a.order || 0) - (b.order || 0));
  ...
  async createEducation(data) {
    const id = generateId();
    const rawCat = data.category !== undefined ? data.category : (data.catgry !== undefined ? data.catgry : "");
    const cleanCat = typeof rawCat === "string" ? rawCat.trim() : (rawCat || "");
    const resolvedCat = cleanCat || (data.degree && data.degree.toLowerCase().includes("cert") ? "Certificate" : "Education");
    const cleanEndDate = data.endDate !== undefined && data.endDate !== null ? data.endDate.trim() : "";
    ...
    if (isUsingRealMysql && pool) {
      await pool.query(
        `INSERT INTO educations (id, institution, degree, field, start_date, end_date, description, location, order_num, category, catgry, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [
          edu.id,
          edu.institution,
  ...
  async updateEducation(id, data) {
    ...
    if (data.category !== undefined) {
      targetCat = typeof data.category === "string" ? data.category.trim() : data.category;
    } else if (data.catgry !== undefined) {
      targetCat = typeof data.catgry === "string" ? data.catgry.trim() : data.catgry;
    }

    const resolvedCat = targetCat !== undefined
      ? targetCat
      : (existing.category || existing.catgry || (existing.degree && existing.degree.toLowerCase().includes("cert") ? "Certificate" : "Education"));

    const resolvedEndDate = data.endDate !== undefined
      ? (data.endDate !== null ? data.endDate.trim() : "")
      : (existing.endDate || "");
    ...
    if (isUsingRealMysql && pool) {
      await pool.query(
        `UPDATE educations SET
         ...
         WHERE id = ?`,
        [
          data.institution !== undefined ? data.institution : null,
